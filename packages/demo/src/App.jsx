...
function App() {
  // Create instances (no more globals!)
  const [instances] = useState(() => createInstances());
  const { streamManager, queryEngine } = instances;
  ...
  useEffect(() => {
    ...
    const existingStreams = streamManager.listStreams();
    const initialFilters = {};
    
    for (const streamName of existingStreams) {
      initialFilters[streamName] = {
        enabled: streamName === '_log' ? true : !streamName.startsWith('_'), // _log enabled by default, other system streams off
        count: 0
      };
    }
    
    // Set initial stream filters
    setStreamFilters(initialFilters);
  ...
  // Add console entry
  const addConsoleEntry = useCallback((command, response) => {
    const newEntry = {
      id: Date.now().toString() + Math.random().toString(36),
      timestamp: new Date(),
      command,
  ...
  const handleStreamToggle = useCallback((streamName) => {
    setStreamFilters(prev => ({
      ...prev,
      [streamName]: {
        ...prev[streamName],
  ...
  const handleMultiStreamToggle = useCallback((selectedStreams) => {
    setStreamFilters(prev => {
      const updated = { ...prev };
  ...
  const handleStatementExecute = useCallback(async (statement, index) => {
    try {
      if (statement.isCommand) {
        const result = await CommandParser.executeCommand(statement.text, streamManager, queryEngine);
